...
const server: SerialPort = new SerialPort({
  path: "COM4",
  baudRate: 115200,
});
...
server.on("readable", () => {
  let serialData: Buffer = server.read();
  serialData = cobs.decode(serialData);

  let msgId = serialData[0];
  let msgData = serialData.slice(1);
  ...
  switch (msgId) {
    case 1:
      ...
      Ping.decode(msgData);
      server.write(cobs.encode(new Uint8Array([2])));
      break;
